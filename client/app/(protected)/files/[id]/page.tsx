...
export default function FileDetailsPage() {
  ...
  const handleDownload = async (disposition: "inline" | "attachment") => {
    ...
    try {
      ...
      const response = await api.get(`/files/${file.id}/download?disposition=${disposition}`, {
        responseType: "blob",
      })
    ...
    return (
      <div className="flex min-h-[400px] items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Loading document details...</p>
        </div>
      </div>
    )
  }
  ...
  if (isError || !file) {
    return (
      <div className="max-w-md mx-auto my-12 text-center p-8 rounded-2xl border border-red-200 dark:border-red-900/60 bg-white dark:bg-slate-900">
        ...
        <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-1">File Not Found</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
          The requested document could not be found or you do not have permission to view it.
        </p>
        <Link href="/files">
          <Button variant="brand" size="sm" leftIcon={<ArrowLeft className="h-4 w-4" />}>
            Back to My Files
          </Button>
        </Link>
      </div>
    ...
    <div className="space-y-8 text-slate-900 dark:text-slate-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <Card className="border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
            ...
            <CardContent className="space-y-4 text-sm divide-y divide-slate-100 dark:divide-slate-800">
              <div className="pt-2 first:pt-0">
                <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">Original Filename</span>
                <p className="font-semibold text-slate-800 dark:text-slate-200 break-all mt-0.5">{file.originalName}</p>
              </div>

              <div className="pt-3">
                <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">File Size</span>
              ...
              <div className="pt-3">
                <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">Verified MIME Type</span>
              ...
              <div className="pt-3">
                <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">SHA-256 Checksum</span>
              ...
              <div className="pt-3">
                <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">Text Extraction Status</span>
                <div className="mt-1">
                  {file.extractionStatus === "DONE" && (
                    <Badge variant="success">DONE (Successfully Extracted)</Badge>
                  )}
              ...
              <div className="pt-3">
                <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">Uploaded Date</span>
              ...
              {file.owner && (
                <div className="pt-3">
                  <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">Owner</span>
                  <p className="font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2 text-xs mt-0.5">
        ...
        <div className="lg:col-span-7 space-y-6">
          <Card className="flex flex-col h-full min-h-[450px] border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
            ...
            <CardContent className="flex-1 flex flex-col justify-center">
              {file.extractedContent ? (
                <div className="relative rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-900 dark:bg-slate-950 p-5 font-mono text-xs text-slate-100 dark:text-slate-100 whitespace-pre-wrap break-words leading-relaxed max-h-[500px] overflow-y-auto shadow-inner">
                  {file.extractedContent}
                </div>
              ) : isImage ? (
                <div className="flex flex-col items-center justify-center p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-center">
                  {imagePreviewUrl ? (
                    <div className="relative max-h-[420px] overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 shadow-md">
                      <img
                        src={imagePreviewUrl}
                    ...
                    <div className="py-8 space-y-3">
                      ...
                      <p className="font-bold text-slate-800 dark:text-white text-sm">Image Asset Ready</p>
                ...
                <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-slate-50/50 dark:bg-slate-950">
                  ...
                  <p className="font-bold text-slate-800 dark:text-white text-sm">No Extracted Content</p>
